<template>
  <chart-card-layout class="chart" :title="`测点加速度（${props.monitoringName}）`">
    <template #extra>
      <div class="filter">
        <div>
          <el-dropdown @command="onUnitChange" size="small">
            <el-link type="primary" :underline="false">
              {{ dictUnit[unitType] }} <el-icon> <ArrowDown /> </el-icon>
            </el-link>
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item command="03">mm/1小时²</el-dropdown-item>
                <el-dropdown-item command="04">mm/1天²</el-dropdown-item>
                <el-dropdown-item command="05">mm/1周²</el-dropdown-item>
                <el-dropdown-item command="07">mm/1月²</el-dropdown-item>
              </el-dropdown-menu>
            </template>
          </el-dropdown>
        </div>
...
import { getAcceleratinfo } from "@/api/slope";
...
const onUnitChange = (command) => {
  unitType.value = command;
  const values = chartSourceData
    .filter((item) => item.timeunit === unitType.value)
    .map((item) => [item.time, item.value]);
  option.series[0].data = values;
  option.yAxis.name = dictUnit[command];
...
onMounted(() => {
  ...
  myChart = echarts.init(document.getElementById("accelerationChartLineChart"));
  myChart.setOption(option);
...
const loadChartData = () => {
  ...
  console.log({ params });
  loading.value = true;
  getAcceleratinfo(params)
    .then((res) => {
      console.log("加速度数据", res);
      const { code, data } = res;
      if (code === 0) {
        // timeunit 30分钟02,1小时03,1天04,1周05,1月07
