...
import hotmapData from "@/assets/data/hongranxingbian.json";
...
class mapUtil {
   ...
   * 加载地图监测点
   */
  loadMonitorPoints(monitors,callback) {
   ...
   * @param {*} option
   */
  loadMineHotmapLayer(option) {
    // this.heatLayer.show = true;
    this.showHeatMap();
  }
  /**
   * 销毁热力图
   */
  hotmapDestroy() {
    if (this.heatLayer && this.heatLayer.state == "added") {
      this.heatLayer.destroy(false);
      this.mapInstance.off(mars3d.EventType.mouseMove);
      this.heatLayer = null;
    }
  }
   ...
   */
  showHeatMap() {
    this.heatLayer && this.heatLayer.remove(true);
    const pointArr = hotmapData.map((item) => {
      item.value = item.value * 10000;
      return item;
    });
    // console.log("热力图数据", pointArr);
    ...
    this.mapInstance.addLayer(this.heatLayer);

    // this.mapInstance.on(mars3d.EventType.mouseMove, (e) => {
    //   const point = mars3d.LngLatPoint.fromCartesian(e.cartesian);
    //   const data = this.heatLayer.getPointData(point);
    //   console.log({ data });
    //   const inhtml = `
    //     经度: ${point.lng} <br />
    //     纬度: ${point.lat} <br />

    //     形变值: ${(data.value / 10 - 30).toFixed(1)} mm`;
    //   this.mapInstance.openSmallTooltip(e.windowPosition, inhtml);
    // });
  }
}
